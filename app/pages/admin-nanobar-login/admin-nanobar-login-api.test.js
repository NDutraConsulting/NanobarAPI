// Integration test for admin-nanobar-login-api.js's fetch wrapper. `fetch` is stubbed (no real network
...
const api = await import("./admin-nanobar-login-api.js");
...
test("submitLogin POSTs the username/password to /admin/nanobar/login with a JSON body", async () => {
  const calls = stubFetch({ status: "success", result: { data: { redirect: "/admin/nanobar/dashboard" } } });

  const envelope = await api.submitLogin("admin", "changeme123");

  assert.equal(calls[0].url, "/admin/nanobar/login");
  assert.equal(calls[0].options.method, "POST");
  assert.equal(calls[0].options.headers["Content-Type"], "application/json");
  assert.equal(calls[0].options.body, JSON.stringify({ username: "admin", password: "changeme123" }));
  assert.equal(envelope.result.data.redirect, "/admin/nanobar/dashboard");
});

...
The shadow app shares the live app's `regression_bricks.db`/`events.db`/`app_admin.db`/
...
def get_replay_app(
    *, db_path: str, events_db_path: str, app_admin_db_path: str, nanobar_admin_db_path: str, blog_db_path: str
) -> NanobarAPI:
    """Returns a memoized shadow app for this exact 5-tuple -- the live app's own db paths,
    which is also the shadow app's natural cache key: tests build a fresh live app (and thus
    fresh temp-dir db paths) per test, so this naturally scopes to "one shadow app per distinct
    live app," never leaking across tests or reusing a stale shadow app pointed at the wrong
    live app's databases.
    """
    # Deferred: app.main -> app.admin.nanobar.routes -> api -> this module, at import time -- a
    # real circular import at module-load, since app/main.py hasn't finished defining
    # build_app() yet when api.py (transitively) imports this file. By the time this function
    # actually runs (request-handling time), app.main is fully initialized.
    from app.main import build_app

    key = (db_path, events_db_path, app_admin_db_path, nanobar_admin_db_path, blog_db_path)

// Integration test for admin-login-api.js's fetch wrapper. `fetch` is stubbed (no real network
...
const api = await import("./admin-login-api.js");
...
test("submitLogin POSTs the username/password to /admin/login with a JSON body", async () => {
  ...
  assert.equal(calls[0].url, "/admin/login");

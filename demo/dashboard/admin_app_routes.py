...
handlers reading the repository layer directly, matching `demo/dashboard/api.py`'s own
established convention for simple reads. The mutating routes go through the full
`NanobarValidatorGate` -> `NanobarController` -> `NanobarService` pipeline (`blog_gates.py`/
`blog_controllers.py`/`blog_services.py`) -- real validation and business logic live there,
unlike the plain reads.
"""
...
from .blog_gates import CreatePostGate, MarkNotificationReadGate, UpdatePostGate
from .blog_repositories import NotificationRepository, PostRepository
from .blog_serialize import notification_to_dict, post_to_dict
...
def build_mount(*, backend: SessionBackend) -> Mount:
    ...
    # substring being present in the path, matching demo/dashboard/api.py's own established
    ...
    # /admin/login instead of getting a 401 JSON envelope.
    return Mount(
        "/admin/app",
        routes=[

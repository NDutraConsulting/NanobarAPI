...
#: Default location, relative to this file, matching every other per-app database's own
#: convention (``demo/data/regression_bricks.db``, ``demo/data/events.db``, ...). ``demo/data/``
#: is gitignored; it may not exist yet or may be empty, and that's fine.
DEFAULT_DB_PATH = Path(__file__).resolve().parent.parent / "data" / "nanobar_type_system.db"
